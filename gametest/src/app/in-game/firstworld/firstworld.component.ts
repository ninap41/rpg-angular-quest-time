...
export class FirstworldComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.Player.worldPoint.length === 0) {
      ...
      this.playersPoint =
        this.Player.worldPoint[this.Player.worldPoint.length - 1];
      this.Check_all();
      this.WorldName = HumanWorldStart.name.replace(
        "CHARHOMETOWN",
        this.Player.hometown
  ...
  nextWorld(direction) {
    if (direction.world1_end) {
      ...
      this.WorldName = this.currentworld.name.replace(
        "CHARHOMETOWN",
        this.Player.hometown
      );
      this.Check_all();
  ...
  check_event_name_home(event) {
    if (event) {
      if (event.enemy) {
        event.enemy.description = event.enemy.description.replace(
          "CHARHOMETOWN",
          this.Player.hometown
        ); // put in service
        event.enemy.description = event.enemy.description.replace(
        ...
        event.enemy.opening_line = event.enemy.opening_line.replace(
          "CHARHOMETOWN",
          this.Player.hometown
        ); // put in service
        event.enemy.opening_line = event.enemy.opening_line.replace(
      ...
      event.description2 = event.description.replace(
        "CHARHOMETOWN",
        this.Player.hometown
      ); // put in service
      event.description2 = event.description.replace(
  ...
  check_name_and_home(point) {
    if (point.description) {
      this.playersPoint.description = this.playersPoint.description.replace(
        "CHARHOMETOWN",
        this.Player.hometown
      );
      this.playersPoint.description = this.playersPoint.description.replace(
    ...
    if (direction.world1_end) {
      ...
      if (this.playersPoint.eventtriggerchance) {
        const trigger = Math.floor(
          Math.random() * this.playersPoint.eventtriggerchance
        );
        if (trigger === 1) {
    ...
    if (direction.world1_end) {
      ...
      if (this.playersPoint.eventtriggerchance) {
        ...
        if (trigger < this.playersPoint.eventtriggerchance) {
          ...
          console.log(
            "this.playersPoint.eventtriggerchance: " +
              this.playersPoint.eventtriggerchance
          );
          this.currentEvent = this.firstworldevents[this.playersPoint.event];
          this.playersPoint.eventtriggerchance = null;
  ...
  use_needed_item(action, idx) {
    ...
    for (let i = 0; i < this.Player.bag.length; i++) {
      if (
        action.needs === this.Player.bag[i].value &&
        this.Player.bag[i].qty > 0
      ) {
      ...
      if (this.currentEvent.influence_event) {
        ...
        if (trigger < this.currentEvent.influence_event.impact_chance) {
          if (this.currentEvent.influence_event.impact[0] === "negative") {
            ...
            this.Player[this.currentEvent.influence_event.impact[1]] -=
              this.currentEvent.influence_event.impact[2];
            this._characterService.global_update_message =
              this.currentEvent.influence_event.description;
          }
          if (this.currentEvent.influence_event.impact[0] === "positive") {
            this.Player[this.currentEvent.influence_event.impact[1]] +=
              this.currentEvent.influence_event.impact[2];
            this._characterService.global_update_message =
              this.currentEvent.influence_event.description3;
          }
        } else {
          this._characterService.global_update_message =
            this.currentEvent.influence_event.description2;
        }
      } else {
  ...
  fight(action) {
    ...
    if (this._battleService.currentEnemy.health <= 0) {
      // base case
      console.log("win");
      ...
      this._audioService.fight_music(false);
      console.log(
        this.currentEvent.description_replace + "UHSAHGSADHGJSADJHGADSGJHAGHJS"
      );
      this._battleService.max_Taunt = 0;
      ...
      if (this.xp > 70) {
        ...
        const restore = ` Stats from before the battle taunts have been restored. You are ${
          70 - this.xp
        }
      ...
      console.log(
        "CURRENT ENEMY LOG: " + JSON.stringify(this._battleService.currentEnemy)
      );
      if (action === "Strike") {
  ...
  post_battle_unequip() {
    for (let i = 0; i < this._characterService.Player.bag.length; i++) {
      if (
        this._characterService.Player.bag[i].name ===
        this._characterService.Player.weapon.name
      ) {
        this._battleService.weaponequip(
          this._characterService.Player.bag[i],
          i,
          "unequip"
        );
        return;
