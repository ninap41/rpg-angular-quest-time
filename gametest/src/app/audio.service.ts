...
export class AudioService {
  ...
  constructor() {
    ...
    if (
      this.current_music.src === null ||
      this.current_music.src === undefined
    ) {
  ...
  initialMusic() {
    // just in case of constructor not working.
  ...
  weaponEquip_sound(state) {
    if (state === "equip") {
      // this.inventory_sound = new Audio();
      this.inventory_sound.src =
        "../../../assets/sounds/rpg/inventory/light_equip.wav";
  ...
  fight_sound(currentEnemy) {
    if (currentEnemy.race === "Orc") {
      this.enemy_sound = new Audio();
      this.enemy_sound.src = `/assets/sounds/rpg/enemy/ogre${
        Math.floor(Math.random() * 4) + 1
      }.wav`;
    } else if (currentEnemy.race === "Human") {
      this.enemy_sound = new Audio();
      this.enemy_sound.src = `/assets/sounds/rpg/enemy/human/human${
